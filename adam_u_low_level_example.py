    ...
    def LowCmdWrite(self):
        if(self.getstate_flag):
            ...
            if self.time_ < self.duration_ * 2 :
                # Swing angles in radians (10% of joint range to be safe)
                L_shoulder_pitch_range = 0.1 * (2.042 - (-3.613))  # ~2.827 rad
                L_shoulder_roll_range = 0.1 * (2.793 - (-0.628))  # ~1.710 rad
                R_shoulder_pitch_range = 0.1 * (2.042 - (-3.613))  # ~2.827 rad
                R_shoulder_roll_range = 0.1 * (0.628 - (-2.793))  # ~1.710 rad
